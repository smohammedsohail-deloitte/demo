...
Terraform is an open-source infrastructure-as-code tool that allows you to define and manage your infrastructure in a declarative way. With Terraform, you can define your infrastructure as code in a configuration file, which can then be used to create, modify, and delete resources in a consistent and repeatable way.

Azure Storage Account is a cloud-based storage solution provided by Microsoft Azure. It allows you to store and manage data objects such as blobs, files, queues, tables, and disks in a scalable, highly available, and durable manner. Here are some key features of Azure Storage Account:

- Blob Storage: Azure Blob Storage is used to store unstructured data such as text and binary data. Blobs can be accessed over HTTP/HTTPS and can be accessed from anywhere in the world.

- File Storage: Azure File Storage is used to store and share files over the Server Message Block (SMB) protocol. It provides fully managed file shares that can be accessed from Windows, macOS, and Linux.

- Queue Storage: Azure Queue Storage is used to store and manage messages. It provides asynchronous message queueing for communication between different components of an application.

- Table Storage: Azure Table Storage is used to store structured NoSQL data such as key-value pairs, and it can be used for storing and querying large amounts of data.

- Disk Storage: Azure Disk Storage is used to store Virtual Hard Disks (VHDs) for virtual machines running on Azure. It provides highly durable and scalable storage for virtual machines.
...
`blob_cors`|blob service cors rules:  https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/storage_account#cors_rule?|map|`{}`
`enable_static_website`|Controls if static website to be enabled on the storage account. Possible values are `true` or `false`|bool|false 
`index_path`|path from your repo root to index.html|string|null 
`custom_404_path`|path from your repo root to your custom 404 page|string|null
